...
int commit_create(const char *message, ObjectID *commit_id_out) {
    ...
    fprintf(stderr, "debug: building tree from index...\n");
    if (tree_from_index(&c.tree) < 0) {
        fprintf(stderr, "debug: tree_from_index failed\n");
        return -1;
    }
    fprintf(stderr, "debug: tree built ok\n");
    ...
    fprintf(stderr, "debug: serialized ok, writing object...\n");

    int rc = object_write(OBJ_COMMIT, data, len, commit_id_out);
    free(data);
    if (rc < 0) {
        fprintf(stderr, "debug: object_write failed\n");
        return -1;
    }
    fprintf(stderr, "debug: object written, updating HEAD...\n");
