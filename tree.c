    ...
    while (i < count) {
        ...
        if (slash) {
            ...
            while (j < count) {
                ...
                for (int d = 0; d < depth; d++) { q = strchr(q, '/') + 1; }
                const char *sl2 = strchr(q, '/');
                if (!sl2) break;
                if ((size_t)(sl2 - q) != dir_name_len) break;
                if (strncmp(q, dir_name, dir_name_len) != 0) break;
                j++;
            }
            ...
            te->name[sizeof(te->name) - 1] = '\0';
            te->hash = entries[i].hash;   // fixed: use .hash not .id
            i++;
        }
    }
